...
let sheetName = 'การตอบแบบฟอร์ม 2';
let pdf_file_name = "เอกสาร - ";
let isSendEmail = true;
let isSendLine = true;
let email_send_default = ['admin@tip-smart.com'];
...
let index_col = {"ประทับเวลา":0,"อีเมล์":1,"เลขรับแจ้ง":2,"วันที่เกิดเหตุ":3,"สถานที่เกิดเหตุ":4,"วันที่เขียน":5,"ชื่อผู้ขับขี่รถประกัน":6,"ทะเบียนรถประกัน":7,"เพื่อให้ทำสัญญา":8,"ทะเบียนคู่กรณี":9,"ค่าซ่อมรถคู่กรณี":10,"จำนวนวันอนุมติ":11,"วันละ":12,"รวมค่าขาดฯ":13,"รายการทรัพย์สิน":14,"ยุติค่าซ่อมจำนวน":15,"ชื่อบุคคล":16,"ยุติสินไหมจำนวน":17,"ชื่อ-สกุลคู่กรณี":18,"อายุ":19,"เลขบัตร":20,"วันออกบัตร":21,"วันหมดอายุ":22,"เชื้อชาติ":23,"สัญชาติ":24,"บ้านเลขที่":25,"หมู่ที่":26,"ชื่อซอย":27,"ชื่อถนน":28,"เขตตำบล":29,"เขตอำเภอ":30,"เขตจังหวัด":31,"เบอร์โทรติดต่อ":32,"การรับมอบอำนาจ":33,"รับมอบจาก":34,"เลขที่":35,"หมู่":36,"ตำบล":37,"อำเภอ":38,"จังหวัด":39,"เบอร์โทรผู้มอบอำนาจ":40,"บัญชีธนาคาร":41,"แบบบัญชี":42,"ออมทรัพย์สาขา":43,"ออมทรัพย์เลขที่":44,"สาขากระแสรายวัน":45,"กระแสรายวันเลขที่":46,"มีรายการเพิ่มหรือไม่":47,"เพื่อจ่ายค่า":48,"รวมเป็นเงิน":49,"อ่านว่า":50,"ใส่ลายเซ็น":51,"ตราประทับบริษัท":52,"เพื่อให้ทำสัญญา[เพื่อจ่ายค่าซ่อมรถคู่กรณี]":53,"เพื่อให้ทำสัญญา[เพื่อจ่ายค่าขาดประโยชน์]":54,"เพื่อให้ทำสัญญา[เพื่อซ่อมทรัพย์สิน]":55,"เพื่อให้ทำสัญญา[เพื่อจ่ายสินไหมทดแทน]":56,"send_email_status":57,"send_line_status":58,"create_pdf_status":59};
...
let colEmailStatusName = "BF";
let colLineStatusName = "BG";
let colPdfStatusName = "BH";
...
let index_col_checkbox = [
  { [index_col["เพื่อให้ทำสัญญา"]]: [{ เพื่อจ่ายค่าซ่อมรถคู่กรณี: "BB" }, { เพื่อจ่ายค่าขาดประโยชน์: "BC" }, { เพื่อซ่อมทรัพย์สิน: "BD" }, { เพื่อจ่ายสินไหมทดแทน: "BE" }] },
];
      ...
      await duplicateSlide().then(async function () {
        await updateCheckboxCol(cur_data, numRow).then(async function () {
          ...
          try {
            ...
            cur_data[index_col["เวลาเกิดเหตุ"]] = customFormatDate(
              cur_data[index_col["เวลาเกิดเหตุ"]],
              "time",
              "HH:mm"
            );
          } catch (e) {}
